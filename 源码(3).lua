...
C2D:Paragraph({Title="防摔伤害",Desc="从高处掉下来不会扣血（外部脚本）",Icon="info"})
C2D:Button({Text="启动防摔伤害",Icon="play",Callback=function()
local ok,err=pcall(function()
loadstring(game:HttpGet("https://raw.githubusercontent.com/cytj777i/Fall-injury/main/防止摔落伤害"))()
end)
if ok then N("防摔","启动成功",2)else N("失败",tostring(err):sub(1,80),4)end
end})
